...
server.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (userExists) {
            console.log(`${username} exists in database, log in successful`);
...
server.post('/signup', async (req, res) => {
    ...
    try {
        ...
        console.log(`Username ${username} has just been stored in database`);
...
server.post('/add-new-note', async (req, res) => {
    ...
    try {
        ...
        if (userExists) {
            ...
            const savedNote = await newNoteToInsert.save();
        ...
        console.error("add note server error", error);
        res.status(500).json({ message: "internal server error" });
    }
});
...
server.post('/fetch-notes', async (req, res) => {
    ...
    try {
        const userNotes = await noteModel.find({ name: username, note: { $ne: null } });
...
server.post('/delete-note', async (req, res) => {
